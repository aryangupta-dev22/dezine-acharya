    ...
    <main className="overflow-x-hidden bg-white text-[#2A1A1A]">
      ...
      <nav className={`fixed top-0 inset-x-0 z-50 h-[72px] flex items-center justify-between px-6 md:px-10 transition-all duration-300 border-b ${scrolled ? "bg-white/95 shadow-sm border-[#f0e0d0]" : "bg-white border-[#f5ede6]"}`}>
        ...
        <div className="hidden md:flex items-center gap-10">
          ...
          <a href="#contact" className="f-dm text-[.82rem] font-medium tracking-[.15em] uppercase bg-[#6B1A1A] text-white px-6 py-2.5 hover:bg-[#4A0E0E] hover:-translate-y-0.5 hover:shadow-lg transition-all duration-300 no-underline">
            Enroll Now
          </a>
        </div>

        <button onClick={() => setMenuOpen(true)} className="md:hidden flex flex-col gap-[5px] bg-transparent border-0 cursor-pointer p-1">
      ...
      <div className={`fixed inset-0 z-[200] bg-white flex-col items-center justify-center gap-8 px-10 ${menuOpen ? "flex" : "hidden"}`}>
        <button onClick={() => setMenuOpen(false)} className="absolute top-6 right-6 bg-transparent border-0 text-2xl text-[#6B1A1A] cursor-pointer">✕</button>
        {["About","Exams","Approach","Community"].map(item => (
          <a key={item} href={`#${item.toLowerCase()}`} onClick={() => setMenuOpen(false)}
             className="f-dm text-xl font-medium tracking-[.12em] uppercase text-[#6B1A1A] no-underline">{item}</a>
        ))}
        <a href="#contact" onClick={() => setMenuOpen(false)}
           className="f-dm text-[.85rem] font-medium tracking-[.15em] uppercase bg-[#6B1A1A] text-white px-8 py-3 no-underline">Enroll Now</a>
      </div>
      ...
      <section className="min-h-screen flex items-center pt-[72px] relative overflow-hidden bg-gradient-to-br from-white via-white to-[#FDF6EC]">
        ...
        <div className="absolute left-[4%] bottom-[12%] w-24 h-24 border border-[#C9922A]/20 rotate-45 pointer-events-none" data-aos="fade-in" data-aos-delay="600" />
        <div className="absolute left-[6%] bottom-[18%] w-12 h-12 border border-[#C9922A]/15 rotate-45 pointer-events-none" data-aos="fade-in" data-aos-delay="800" />
        ...
        <div className="max-w-[1200px] mx-auto px-6 md:px-10 py-20 grid grid-cols-1 md:grid-cols-2 gap-14 md:gap-20 items-center w-full">
          ...
          <div data-aos="fade-right" data-aos-delay="100">
            ...
            <h1 className="f-play font-black text-5xl md:text-[4rem] leading-[1.1] text-[#4A0E0E]">Where Design</h1>
            <h1 className="f-corm font-light italic text-5xl md:text-[4rem] leading-[1.1] text-[#C9922A] my-1">is Discovered,</h1>
            <h1 className="f-play font-black text-5xl md:text-[4rem] leading-[1.1] text-[#4A0E0E] mb-8">Not Drilled.</h1>

            {/* Shimmer accent bar */}
            <div className="shimmer-bar h-[3px] w-24 mb-8 rounded-full" data-aos="fade-right" data-aos-delay="300" />

            <p className="f-dm text-[1rem] leading-relaxed text-[#8A6A5A] font-light max-w-[440px] mb-10">
              Mentoring aspirants for <strong className="text-[#6B1A1A] font-semibold">NID, NIFT & UCEED</strong> through creative intuition, structured guidance, and reflective mentorship — transforming exam pressure into a meaningful design journey.
            </p>

            <div className="flex gap-4 flex-wrap" data-aos="fade-up" data-aos-delay="350">
              <a href="#contact" className="f-dm text-[.85rem] font-medium tracking-[.15em] uppercase bg-[#6B1A1A] text-white px-9 py-4 hover:bg-[#4A0E0E] hover:-translate-y-0.5 hover:shadow-xl transition-all duration-300 no-underline">
                Begin Your Journey
              </a>
              <a href="#approach" className="f-dm text-[.85rem] font-medium tracking-[.15em] uppercase border border-[#6B1A1A] text-[#6B1A1A] px-9 py-4 hover:bg-[#6B1A1A] hover:text-white transition-all duration-300 no-underline">
                Our Approach
              </a>
            </div>

            {/* Stats */}
            <div className="mt-12 flex gap-10 flex-wrap" data-aos="fade-up" data-aos-delay="450">
              {[{num:"1L+",label:"Aspirants Goal"},{num:"3",label:"Design Exams"},{num:"∞",label:"Creative Potential"}].map((s,i) => (
                <div key={s.label} className="stat-anim" style={{animationDelay:`${i*120}ms`}}>
                  <div className="f-corm font-bold text-[3.6rem] text-[#6B1A1A] leading-none">{s.num}</div>
                  <div className="f-dm text-[.75rem] tracking-[.15em] uppercase text-[#8A6A5A] mt-1">{s.label}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div data-aos="fade-left" data-aos-delay="250" className="relative">
            <div className="bg-gradient-to-br from-[#6B1A1A] to-[#4A0E0E] p-11 relative overflow-hidden">
              ...
              <p className="f-corm font-light italic text-[1.6rem] leading-relaxed text-white mb-8">
                "To democratize design education in India — making meaningful mentorship accessible, inspiring, and free from commercial exploitation."
              </p>
              <div className="border-t border-[#C9922A]/30 pt-6 flex gap-3 flex-wrap">
                {["NID","NIFT","UCEED"].map(e => (
                  <span key={e} className="f-dm text-[.78rem] tracking-[.2em] border border-[#C9922A]/50 text-[#E8B84B] px-4 py-1.5">{e}</span>
                ))}
              </div>
            </div>
        ...
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 opacity-40 scroll-hint">
          <div className="f-dm text-[.65rem] tracking-[.25em] uppercase text-[#6B1A1A]">Scroll</div>
      ...
      <section id="exams" className="bg-[#FDF6EC] py-24 px-6 md:px-10">
        <div className="max-w-[1200px] mx-auto">
          <div data-aos="fade-up" className="text-center mb-16">
            <p className="f-dm text-[.75rem] tracking-[.3em] uppercase text-[#C9922A] font-medium mb-4">Entrance Exams</p>
            ...
            <p className="f-dm text-[1rem] text-[#8A6A5A] font-light max-w-md mx-auto leading-relaxed">
              India's most prestigious design entrance examinations — approached not as a test, but as a creative milestone.
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-0.5">
            {[
              {code:"NID",   full:"National Institute of Design",                 desc:"India's premier design institution. NID entrance demands creative originality, design thinking and spatial reasoning.", icon:"◈", featured:false},
              {code:"NIFT",  full:"National Institute of Fashion Technology",      desc:"Fashion, textiles and design — NIFT CAT tests visual sensitivity, creative aptitude and situational judgment.",       icon:"◇", featured:true },
              {code:"UCEED", full:"Undergraduate Common Entrance Exam for Design", desc:"IIT design programs. UCEED evaluates visualization, environment and social awareness, and analytical reasoning.",     icon:"◉", featured:false},
            ].map((exam, i) => (
              <div key={exam.code} data-aos="fade-up" data-aos-delay={i*120}
                ...
                <div className={`f-corm text-[2.5rem] mb-4 opacity-60 transition-colors duration-300 ${exam.featured ? "text-[#E8B84B]" : "text-[#C9922A] group-hover:text-[#E8B84B]"}`}>{exam.icon}</div>
                <div className={`f-play font-bold text-[2rem] mb-2 transition-colors duration-300 ${exam.featured ? "text-[#E8B84B]" : "text-[#6B1A1A] group-hover:text-[#E8B84B]"}`}>{exam.code}</div>
                <div className={`f-dm text-[.75rem] tracking-[.1em] uppercase mb-5 transition-colors duration-300 ${exam.featured ? "text-white/60" : "text-[#8A6A5A] group-hover:text-white/60"}`}>{exam.full}</div>
                <p   className={`f-dm text-[.92rem] leading-relaxed font-light transition-colors duration-300 ${exam.featured ? "text-white/85" : "text-[#8A6A5A] group-hover:text-white/80"}`}>{exam.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="about" className="py-24 px-6 md:px-10 bg-white">
        <div className="max-w-[1200px] mx-auto grid grid-cols-1 md:grid-cols-2 gap-14 md:gap-20">
          ...
          <div data-aos="fade-right">
            <p className="f-dm text-[.75rem] tracking-[.3em] uppercase text-[#C9922A] font-medium mb-4">The Challenge</p>
            ...
            <div className="flex flex-col gap-4">
              {[
                ...
                <div key={i} data-aos="fade-right" data-aos-delay={i*100}
                     className="prob-card border-l-[3px] border-[#C9922A] pl-6 py-5 bg-[#FDF6EC] hover:border-[#6B1A1A] hover:translate-x-1">
                  <div className="flex gap-3 items-start">
                    <span className="f-corm text-[#C9922A] text-xl shrink-0 mt-0.5">{String(i+1).padStart(2,"0")}</span>
                    <p className="f-dm text-[.93rem] leading-relaxed text-[#2A1A1A] font-light">{problem}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div data-aos="fade-left">
            <p className="f-dm text-[.75rem] tracking-[.3em] uppercase text-[#C9922A] font-medium mb-4">Our Solution</p>
            <h2 className="f-play font-bold text-3xl md:text-[2.5rem] text-[#4A0E0E] mb-9">The Dezine Acharya Difference</h2>
            <div>
              {[
                {title:"Mentorship over Teaching", desc:"Personalized creative guidance that sees the individual behind the aspirant.", icon:"◎"},
                {title:"Think, Don't Rote",        desc:"A curriculum built on design thinking, not repetitive practice drills.",     icon:"◈"},
                {title:"Affordable & Flexible",    desc:"Breaking the cycle of commercial exploitation in coaching.",                  icon:"◇"},
                {title:"Community-Driven",         desc:"Peer feedback, shared growth, and continuous engagement across platforms.",   icon:"◉"},
                {title:"Always Accessible",        desc:"YouTube, Instagram & live communities — design education without walls.",     icon:"◐"},
              ].map((item, i) => (
                <div key={item.title} data-aos="fade-up" data-aos-delay={i*80}
                  ...
                  <span className="text-[#C9922A] text-2xl shrink-0 mt-0.5 group-hover:scale-110 transition-transform duration-200">{item.icon}</span>
                  <div>
                    <div className="f-dm font-semibold text-[.93rem] text-[#6B1A1A] mb-1">{item.title}</div>
                    <div className="f-dm text-[.88rem] text-[#8A6A5A] leading-relaxed font-light">{item.desc}</div>
                  </div>
                </div>
              ))}
            </div>
      ...
      {/* ══════════════════════════════════
          MISSION
      ══════════════════════════════════ */}
      <section id="approach" className="bg-gradient-to-br from-[#4A0E0E] to-[#6B1A1A] py-24 px-6 md:px-10 relative overflow-hidden">
        <div className="absolute -top-24 -right-24 w-96 h-96 rounded-full border border-[#C9922A]/15 pointer-events-none float-anim" style={{animationDelay:"1s"}} />
        ...
        <div data-aos="zoom-in" className="max-w-[900px] mx-auto text-center relative">
          <p className="f-dm text-[.75rem] tracking-[.3em] uppercase text-[#E8B84B] opacity-80 mb-5">Our Mission</p>
          <h2 className="f-corm font-light italic text-4xl md:text-[3.2rem] text-white leading-relaxed mb-10">
            "To guide one lakh design aspirants across India through a human-centered, experience-driven learning model — where design is{" "}
            <span className="text-[#E8B84B]">discovered</span>, not drilled."
          </h2>
          <div data-aos="fade-up" data-aos-delay="200" className="flex justify-center gap-4 flex-wrap">
      ...
      <section id="community" className="bg-[#FDF6EC] py-24 px-6 md:px-10">
        <div className="max-w-[1200px] mx-auto">
          <div data-aos="fade-up" className="text-center mb-16">
            <p className="f-dm text-[.75rem] tracking-[.3em] uppercase text-[#C9922A] font-medium mb-4">Student Voices</p>
            ...
            {[
              ...
              <div key={t.name} data-aos="fade-up" data-aos-delay={i*120}
                ...
                <div className="border-t border-[#f0e0d0] pt-4 flex justify-between items-center">
                  ...
                  <span className="f-dm text-[.72rem] tracking-[.15em] uppercase text-[#C9922A]">{t.exam}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* ══════════════════════════════════
          CTA
      ══════════════════════════════════ */}
      <section id="contact" className="py-24 px-6 md:px-10 bg-white text-center">
        <div data-aos="fade-up" className="max-w-[680px] mx-auto">
          ...
          <div className="flex gap-4 justify-center flex-wrap">
            <a href="#" className="f-dm text-[.85rem] font-medium tracking-[.15em] uppercase bg-[#6B1A1A] text-white px-10 py-4 hover:bg-[#4A0E0E] hover:-translate-y-0.5 hover:shadow-xl transition-all duration-300 no-underline">
              Enroll Now
            </a>
            <a href="https://www.instagram.com/dezine_acharya" target="_blank" rel="noopener noreferrer"
      ...
      <footer className="bg-[#3A0A0A] text-white pt-16 pb-8 px-6 md:px-10">
        <div className="max-w-[1200px] mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-[2fr_1fr_1fr] gap-12 md:gap-16 mb-14">
            ...
            <div data-aos="fade-up" data-aos-delay="0">
              <div className="flex items-center gap-3 mb-5">
                <span className="f-play font-black text-[1.4rem] text-white">DEZINE</span>
                <span className="w-px h-5 bg-[#C9922A] opacity-60" />
                <span className="f-corm text-[1rem] text-[#C9922A] tracking-[.15em]">Acharya</span>
              </div>
              <p className="f-dm text-[.9rem] leading-[1.9] text-white/75 font-light max-w-[300px] mb-6">
                An experiential learning platform mentoring students for NID, NIFT, and UCEED — where design is discovered, not drilled.
              </p>
              {/* Social chips */}
              <div className="flex gap-3 flex-wrap">
                ...
                <a href="https://wa.me/12345678" target="_blank" rel="noopener noreferrer"
            ...
            <div data-aos="fade-up" data-aos-delay="100">
              <p className="f-dm text-[.72rem] tracking-[.28em] uppercase text-[#C9922A] mb-6 pb-3 border-b border-[#C9922A]/20">Exams</p>
              {["NID Preparation","NIFT Preparation","UCEED Preparation","Design Fundamentals"].map(link => (
                <div key={link} className="mb-4 flex items-center gap-2 group">
                  ...
                  <a href="#" className="f-dm text-[.88rem] text-white/70 font-light no-underline hover:text-white transition-colors duration-200">{link}</a>
                </div>
              ))}
            </div>
            ...
            <div data-aos="fade-up" data-aos-delay="200">
              <p className="f-dm text-[.72rem] tracking-[.28em] uppercase text-[#C9922A] mb-6 pb-3 border-b border-[#C9922A]/20">Connect</p>
              {[
                ...
                <div key={link.label} className="mb-4 flex items-center gap-2 group">
                  <span className="w-1 h-1 rounded-full bg-[#C9922A] opacity-60 group-hover:opacity-100 transition-opacity" />
          ...
          <div className="border-t border-white/10 pt-6 flex flex-wrap justify-between items-center gap-3">
            <span className="f-dm text-[.78rem] text-white/50 font-light">© 2024 Dezine Acharya. All rights reserved.</span>
